...
class FilterPhotons {
public:
  // Function to filter out tracks that do not have enough photons in the
  // relevant Ckov zone from pdg
  std::pair<int, int> expectedNumPhotons(double n, double p, int pdg) {
    ...
    double numExpectedPhotons =
        sinThetaC * sinThetaC * NUM_EXPECTED_PHOTONS_SATURATION / 0.4;

    Printf("pdg %d num exp phot %.3f", pdg, numExpectedPhotons);

    // Calculate limits for evaluation using Poisson distribution mean - std
    int limitEvaluate =
        std::floor(numExpectedPhotons - std::sqrt(numExpectedPhotons));
    int limitEvaluateHalf =
        std::floor(numExpectedPhotons / 2 - std::sqrt(numExpectedPhotons / 2));
  ...
  // EF > TODO make the sigmaRefIndex on sigmaRefIndex =
  // sigmaRefIndex/sqrt(expectedPhotonsPerSpecie) ??
  double momThresh(double mass, double refIndex) {
    ...
    auto meanRefIndex = refIndex; // ef > todo change notation to avoid
                                  // disambiguity >> IP to func meanRefIndex
    auto sigmaRefIndex =
        0.0055; // ef > TODO make this more accurate and not hardcoded
  ...
  bool evaluateMomentumThreshold(bool &momThreshPi, bool &momThreshKa,
                                 bool &momThreshPr, double p, double n) {
  ...
  bool evaluateNumPhotThresh(const std::vector<float> &pr_phi,
                             const std::vector<float> &k_phi,
                             const std::vector<float> &pi_phi, double n,
                             double p, int pdgCodeTrack,
                             bool fixedPdgIsNumPhotsOk) {
    ...
    // function to count photons in the half plane
    ...
    auto [numPhotThreshProton, numPhotHalfThreshProton] =
        expectedNumPhotons(n, p, 2212);
    auto [numPhotThreshKaon, numPhotHalfThreshKaon] =
        expectedNumPhotons(n, p, 321);
    auto [numPhotThreshPion, numPhotHalfThreshPion] =
        expectedNumPhotons(n, p, 211);
    ...
    Printf("Pr %d hlf %d mom %d", (numProtons >= numPhotThreshProton),
           (numPhotonInHalfProton2 >= numPhotHalfThreshProton), momThreshPr);
    Printf("Ka %d hlf %d mom %d", (numKaons >= numPhotThreshKaon),
           (numPhotonInHalfKaon2 >= numPhotHalfThreshKaon), momThreshKa);
    Printf("Pi %d hlf %d mom %d", (numPions >= numPhotThreshPion),
           (numPhotonInHalfPion2 >= numPhotHalfThreshPion), momThreshPi);
    */

    // Check if thresholds are met for each particle type
    bool condPr = (numProtons >= numPhotThreshProton) &&
                  (numPhotonInHalfProton2 >= numPhotHalfThreshProton) &&
                  (momThreshPr);
    bool condKa = (numKaons >= numPhotThreshKaon) &&
                  (numPhotonInHalfKaon2 >= numPhotHalfThreshKaon) &&
                  (momThreshKa);
    bool condPi = (numPions >= numPhotThreshPion) &&
                  (numPhotonInHalfPion2 >= numPhotHalfThreshPion) &&
                  (momThreshPi);
